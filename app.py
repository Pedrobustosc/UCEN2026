# app.py — Salas UCEN 2026 · Versión refactorizada
# Cambios vs. original:
#   · Datos extraídos a data_source.py (b, FAC_SHORT, ED_SHORT, MODULOS, etc.)
#   · @st.cache_data en load_period / load_rev
#   · to_xlsx corregido: BytesIO + openpyxl (archivo .xlsx nativo)
#   · Capacidad casteada estrictamente a int antes de comparaciones
#   · Inputs de texto sanitizados (re.escape) antes de usarse en str.contains
#   · try-except robusto en decode Base64 para interceptar datos corruptos
...
# Update badge — fecha hardcodeada desde data_source, sin interpolación de input usuario
st.markdown(
    f'<div class="update-badge">🕐 Última actualización<br><span>{b["update_dt"]}</span></div>',
    unsafe_allow_html=True,
)
...
def leg(txt):
    # txt siempre es string literal del código fuente — nunca contiene input de usuario
    st.markdown('<div class="legend-box">📖 ' + txt + '</div>', unsafe_allow_html=True)

def fac_c(df, col="Facultad"):
    df[col + "_c"] = df[col].map(FAC_SHORT).fillna(df[col])
    return df

def to_xlsx(df: pd.DataFrame) -> bytes:
    """Genera un archivo Excel .xlsx nativo usando openpyxl vía BytesIO.
    Preserva tildes y caracteres especiales correctamente."""
    buf = io.BytesIO()
...
def _safe_decode(key: str) -> pd.DataFrame:
    """Decodifica una clave Base64 del diccionario b de forma segura.
    Lanza ValueError con mensaje claro si el dato está corrupto."""
    try:
        raw = base64.b64decode(b[key])
        return pd.read_csv(io.StringIO(raw.decode("utf-8")))
    except KeyError:
        raise ValueError(f"Clave '{key}' no encontrada en el origen de datos.")
    except (base64.binascii.Error, UnicodeDecodeError) as e:
        raise ValueError(f"Error al decodificar '{key}': {e}")
    except Exception as e:
        raise ValueError(f"Error procesando '{key}': {e}")

# ── CARGA CON CACHÉ ───────────────────────────────────────────────────────────
@st.cache_data
def load_period(p: str) -> dict:
    """Carga y decodifica todos los DataFrames de un período.
    Ejecuta UNA SOLA VEZ por período gracias a @st.cache_data."""
    ...
    for k in keys:
        ...
        try:
            ...
            # Garantizar que Capacidad sea int donde exista
            if "Capacidad" in df.columns:
                df["Capacidad"] = pd.to_numeric(df["Capacidad"], errors="coerce").fillna(0).astype(int)
            result[k] = df
        except ValueError as e:
            st.error(f"⚠️ No se pudo cargar el bloque de datos '{full_key}': {e}")
            result[k] = pd.DataFrame()
    return result

@st.cache_data
def load_rev() -> pd.DataFrame:
    """Carga el DataFrame de reservas. Cacheable y seguro."""
    try:
        return _safe_decode("rev")
    except ValueError as e:
        st.error(f"⚠️ No se pudo cargar las reservas: {e}")
        return pd.DataFrame()
...
if sel_periodo.startswith("2026-01"):
    periodos = ["202601"]; label_periodo = "2026-01"
elif sel_periodo.startswith("2026-02"):
    periodos = ["202602"]; label_periodo = "2026-02"
else:
    periodos = ["202601", "202602"]; label_periodo = "Anual 2026"
...
ocup_ed = (
    df_res.drop_duplicates(["Edificio", "Periodo"] if "Periodo" in df_res.columns else ["Edificio"])
...
leg("<b>Bloques posibles:</b> combinaciones sala×día×módulo disponibles en el período. <b>Bloques ocupados:</b> cuántos tienen al menos una clase asignada. <b>Ocupación global = bloques ocupados / bloques posibles × 100.</b>")
st.divider()
...
with tabs[0]:
    if not fr.empty:
        ...
        leg("<b>% Ocupación por módulo:</b> promedio de (salas ocupadas / total salas) en ese módulo durante la semana. <b>Línea roja 90%:</b> umbral crítico — sobre este valor el edificio está saturado en ese módulo.")
        ...
        leg("<b>Heatmap:</b> cada celda = % ocupación de un edificio en un módulo. Verde oscuro = mucha disponibilidad. Rojo = saturación.")
        ...
        leg("<b>Salas libres vs ocupadas:</b> promedio de salas en cada estado por módulo, sumando todos los edificios.")
...
with tabs[1]:
    if not fr.empty:
        ...
        if not fr2.empty:
            ...
            leg("<b>Salas libres por día y módulo:</b> cantidad de salas disponibles en ese edificio para cada combinación día-módulo.")
...
with tabs[2]:
    if not fd.empty:
        ...
        if not fd3.empty:
            ...
            leg("<b>Distribución de cupos disponibles:</b> de las salas libres en ese edificio y día, cuántas hay de cada rango de capacidad.")
            c1, c2 = st.columns(2)
            with c1:
                ...
                leg("<b>SC vs Laboratorio:</b> laboratorios tienen PCs y equipamiento especializado — no son intercambiables con una sala estándar.")
            with c2:
                ...
                leg("<b>Cupos totales disponibles:</b> suma de capacidad de todas las salas libres en cada módulo.")
...
with tabs[3]:
    ...
    leg("<b>Bloques de sala usados:</b> cada bloque = una sala ocupada en un día y módulo específico. <b>Exceso de cupo:</b> secciones donde el cupo supera la capacidad de la sala.")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        ...
        leg("<b>Cupo vs capacidad sala:</b> línea roja diagonal = equilibrio perfecto. Puntos sobre la línea = cupo supera la sala (problema urgente).")
...
with tabs[4]:
    ...
    leg("<b>Horas semanales:</b> suma de minutos de las asignaturas del docente / 60. <b>Conflicto horario:</b> el docente aparece en 2+ clases el mismo día y hora.")
    ...
    with col_d1:
        ...
        leg("<b>Distribución de carga:</b> concentración a la derecha indica sobrecarga en parte del cuerpo docente.")
    with col_d2:
        ...
        leg("<b>Horas promedio por facultad:</b> compara la intensidad de carga entre unidades.")
    ...
    if busq_doc:
        # Sanitización: re.escape previene inyección de regex en str.contains
        safe_query = re.escape(busq_doc.strip())
        res_doc = df_doc[
            df_doc["Docente"].str.contains(safe_query, case=False, na=False, regex=True) |
            df_doc["RUT"].astype(str).str.contains(safe_query, na=False, regex=True)
        ]
...
with tabs[5]:
    ...
    if label_periodo == "Anual 2026":
        ...
        for i, p in enumerate(periodos):
            ...
            fig_pd = px.bar(df_pd_fac.sort_values("Secciones", ascending=True), x="Secciones", y="Fac_corto",
                            color="TIPO_PENDIENTE", orientation="h", barmode="stack",
        ...
        leg("<b>Por designar:</b> la celda contiene 'POR DESIGNAR'. <b>Sin asignar:</b> celda completamente vacía.")
...
with tabs[6]:
    ...
    leg("<b>Próximas a iniciar:</b> secciones cuya primera clase ocurre dentro del rango seleccionado. <b>Sin docente:</b> requieren regularización urgente.")
    c1, c2 = st.columns(2)
    with c1:
...
with tabs[7]:
    ...
    leg("<b>Conflicto de docente:</b> el mismo RUT aparece en 2+ clases en el mismo día y hora. <b>Conflicto de sala:</b> la misma sala tiene 2+ secciones en el mismo bloque.")
...
with tabs[8]:
    if label_periodo == "Anual 2026":
        ...
        leg("<b>Ratio = Cupo sección / Capacidad sala × 100.</b> <b>Muy baja (<30%):</b> sala sobredimensionada. <b>Baja (30-50%):</b> sala grande para el grupo.")
        ...
        leg("<b>Eficiencia por edificio:</b> distribución de clases por categoría de uso. Mucho rojo/naranja = muchas secciones en salas demasiado grandes.")
        ...
        leg("<b>Optimización:</b> secciones que ocupan una sala más grande de lo necesario y para las que existe una sala más pequeña disponible en el mismo edificio, día y módulo.")
        if not df_opt.empty and "Asignatura" in df_opt.columns:
            ...
            rename_map = {"ID_Asig": "ID Asig.", "Asignatura": "Asignatura", "Seccion": "Secc.", "Tipo": "Tipo",
                          "Facultad_c": "Facultad", "Edificio": "Edificio", "Dia": "Día", "Modulo": "Mód.",
                          "Sala_actual": "Sala act.", "Cap_actual": "Cap. act.", "Cupo": "Cupo",
                          "Ratio_actual": "Ratio act. %", "Sala_sugerida": "Sala sug.", "Cap_sugerida": "Cap. sug.",
                          "Ratio_nuevo": "Ratio nuevo %", "Mejora_ratio": "Mejora %"}
...
with tabs[9]:
    if label_periodo == "Anual 2026":
        ...
        p = periodos[0]
        cm  = data[p]["conc_mod"].copy()
        cfm = data[p]["conc_fac_mod"].copy()
        ct  = data[p]["conc_turno"].copy()
        ...
        with c1:
            ...
            leg("<b>Clases por módulo:</b> total de clases programadas en cada módulo horario.")
        with c2:
            ...
            leg("<b>Diurno vs vespertino:</b> distribución de clases por jornada y facultad.")
        ...
        leg("<b>Clases por módulo y facultad:</b> muestra si alguna facultad concentra toda su actividad en ciertos módulos.")
...
with tabs[10]:
    ...
    if busq_r:
        # Sanitización con re.escape antes de usar en str.contains
        safe_busq = re.escape(busq_r.strip())
        frv = frv[
            frv["Sala"].str.contains(safe_busq, case=False, na=False, regex=True) |
            frv["Motivo"].str.contains(safe_busq, case=False, na=False, regex=True) |
            frv["Descripcion"].str.contains(safe_busq, case=False, na=False, regex=True)
        ]
    ...
    leg("<b>Reservas:</b> solicitudes puntuales fuera de la planificación semestral regular — eventos, cursos cortos, diplomados.")
    c1, c2 = st.columns(2)
    with c1:
    ...
    leg("<b>Top 15 salas más reservadas:</b> salas con mayor demanda puntual.")
...
with tabs[11]:
    ...
    # Cast a int garantizado antes de la comparación con slider
    fd4 = fd[
        (fd["Ed_corto"] == sel4e) &
        (fd["Dia"] == sel4d) &
        (fd["Modulo"] == sel4m) &
        (fd["Capacidad"].astype(int) >= cupo_min) &
        (fd["Tipo"].isin(sel_tipo))
    ...
    if not fd4.empty:
        ...
        leg("<b>Cómo usar esta tabla:</b> selecciona edificio, día y módulo donde necesitas ubicar una sección. Filtra por capacidad mínima según el cupo.")
